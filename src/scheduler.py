...
def is_today_trading_day() -> bool:
    """
    Check if today is a Chinese stock market trading day using exchange_calendars.
    ...
    try:
        ...
        logger.warning(
            "exchange_calendars 库未安装，无法准确判断交易日。请执行: pip install exchange_calendars"
        )
        import datetime
        ...
        logger.warning(
            f"Failed to check trading day: {e}. Assuming today is a trading day."
        )
        return True
...
class Scheduler:
    ...
    def _safe_run_task(self):
        ...
        if not is_today_trading_day():
            logger.info(
                f"今日 ({datetime.now().strftime('%Y-%m-%d')}) 非交易日，跳过定时任务"
            )
            return
        else:
            logger.info(
                f"今日 ({datetime.now().strftime('%Y-%m-%d')}) 是交易日，执行定时任务"
            )
...
def run_with_schedule(
    task: Callable, schedule_time: str = "18:00", run_immediately: bool = True
):
    """
    便捷函数：使用定时调度运行任务
...
if __name__ == "__main__":
    ...
    run_with_schedule(test_task, schedule_time="18:59", run_immediately=True)
